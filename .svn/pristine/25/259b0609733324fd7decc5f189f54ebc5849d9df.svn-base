...
<body>
	<div class="page block-col">
		...
		<div class="center block-row">
			...
			<div class="left-menu bkcolor-black color-white" v-if="leftMenu" >
				...
				<a class="left-menu-item left-menu-item-active" href="user" v-if="{{ Session::get('type')}} == 1">
					<span class="icon fa-user left-menu-icon"></span>
					用户				
				</a>
				<a class="left-menu-item" href="coupons" v-if="{{ Session::get('type')}} != 2">
					<span class="icon fa-credit-card left-menu-icon"></span>
					优惠券				
				</a>
				<a class="left-menu-item" href="userCoupons" v-if="{{ Session::get('type')}} != 2">
	...
	function addItem(obj,callback){
		...
		// setTimeout(function(){
		// 	dialogconfig.showloading=false;
		// 	//添加应该返回主键的值，暂时用下面的代码模拟，实际使用改成传来的
		// 	var pk=vm.list.length;
		// 	if(callback){
		// 		callback(pk);
		// 	}
		// },1000);
	...
	function downList(callback){
		...
		//暂时用延时函数模拟
		$.ajax({
			url:'http://juke.saoyidian.cn/zht/public/admin/getUser',
			type:'get',
			success:function (res) {
				for(v in res) {
					res[v]['nickname'] = base.decode(res[v]['nickname']);
				}
