...
<body>
	<div class="page block-col">
		...
		<div class="center block-row">
			...
			<div class="left-menu bkcolor-black color-white" v-if="leftMenu" >
				...
				<a class="left-menu-item left-menu-item-active" href="userCoupons">
			...
			<div class="main bkcolor-gray block-col">
				<div class="main-title bkcolor-white">
					<form @submit.prevent="myScreen()" style="display: initial;">
						订单号: <input type="text" v-model="screen.order_num">
						使用时间: <input type="datetime-local" v-model="screen.usetime">
						手机号: <input type="text" v-model="screen.phone">
						优惠券名: <input type="text" v-model="screen.name">
				...
				<div class="main-content  bkcolor-white">
					<table role="table">
						<thead role="rowgroup">
							<tr role="row">
							<th role="columnheader" v-if="{{ Session::get('type')}} == 1">用户id</th>
							<th role="columnheader">手机</th>
							<th role="columnheader" v-if="{{ Session::get('type')}} == 1">优惠券id</th>
							<th role="columnheader">优惠券名称</th>
							<th role="columnheader">领取时间</th>
							<th role="columnheader">使用时间</th>
							<th role="columnheader">订单号</th>
							</tr>
						</thead>
						<tbody role="rowgroup">
							<tr role="row" v-for="(item,id) in list" v-if="id+(page-1)*num>=(page-1)*num && id+(page-1)*num<page*num">

								<td role="cell" v-if="{{ Session::get('type')}} == 1">@{{ item.userid }}</td>
								<td role="cell">@{{ item.phone }}</td>
								<td role="cell" v-if="{{ Session::get('type') }} == 1">@{{ item.coupon_id }}</td>
								<td role="cell">@{{ item.coupon_name }}</td>
								<td role="cell">@{{ item.cou_addtime }}</td>
								<td role="cell">@{{ item.usetime }}</td>
								<td role="cell">@{{ item.order_num }}</td>
								<!-- <td role="cell">
									<span @click="editClick(item,id)" class="icon fa-pencil-square-o" style="margin-left:10px;color:#15d03e;"></span>
			...
			<div class="my-dialog bkcolor-white" v-if="'my-dialog'==dialogconfig.handle">
				...
				<form @submit.prevent="mySubmit()" class="line-layout" style="flex:1;overflow:auto;">
					...
					<div class="form-div col-12">
						<div class="form-title col-2">name</div>
						<input type="text" class="form-input" v-model="formItem.name" />
					</div>
					<div class="form-div col-12">
						<div class="form-title col-2">level</div>
						<input type="text" class="form-input" required v-model="formItem.level" />
					</div>
					<div class="form-div col-12">
						<div class="form-title col-2">area</div>
						<input type="text" class="form-input" v-model="formItem.area" />
					</div>
					<div class="form-div col-12">
						<div class="form-title col-2">cs_id</div>
						<input type="text" class="form-input" v-model="formItem.cs_id" />
					</div>
					<div class="form-div col-12">
						<div class="form-title col-2">Wars or Trek?</div>
						<input type="text" class="form-input" v-model="formItem.Wars_Trek" />
					</div>
					<div class="form-div col-12">
						<div class="form-title col-2">Secret Alias</div>
						<input type="text" class="form-input" v-model="formItem.Secret_Alias" />
					</div>
	...
	function downList(obj,callback){
		...
		var base = new Base64();
		$.ajax({
			url:'http://juke.saoyidian.cn/zht/public/admin/getdata',
			data:{
				'usetime': obj.usetime,
				'phone': obj.phone,
				'name': obj.name,
