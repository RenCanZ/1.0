...
<body>
						...
						<tbody role="rowgroup">
							<tr role="row" v-for="(item,id) in list" v-if="id+(page-1)*num>=(page-1)*num && id+(page-1)*num<page*num">
								...
								<td role="cell">
									...
									<span v-if="{{ Session::get('type')}} != 2" title="添加优惠券" @click="addCouClick(item,id)" class="icon fa-credit-card" style="margin-left:10px;color:#15d03e;"></span>							
								</td>
							</tr>
		...
		<div class="popup-level" v-if="!dialogconfig.isClose">
			...
			<div class="my-dialog bkcolor-white" v-if="'my-dialog'==dialogconfig.handle">
				...
				<form @submit.prevent="mySubmit()" class="line-layout" style="flex:1;overflow:auto;">
					...
					<div class="form-div col-12"  v-if="{{ Session::get('type')}} != 3">
	...
	var vm=new Vue({
		...
		methods:{
			...
			loadList:function(){
				//1 餐饮 2 酒店 3 服务 4 零售
				var _this=this;
				var type = {1: '餐饮',2: '酒店',3: '服务',4: '零售'};
				downList(this.screen,function(list){
					console.log(list.program);
					for(id in list.program) {
						for(k in type) {
							if(k == list.program[id].type) {
								list.program[id].type = type[k];
							}
						}
					}
	...
	function saveItem(obj,callback){
		// console.log(obj);
		$.ajax({
			url:'http://juke.saoyidian.cn/zht/public/admin/saveprogram',
			data:{
				'appid' : obj.appid,
	...
	function delItem(pid,callback){
		$.ajax({
			url:'http://juke.saoyidian.cn/zht/public/admin/delprogram',
			data:{
				'id' : pid,
			},
			type:'get',
	...
	function downList(obj,callback){
		var list=[];
		$.ajax({
			url:'http://juke.saoyidian.cn/zht/public/admin/getprogram',
			data:{
				'appid' : obj.appid,
